...
SysTick   ├──┬──┬──┬──┬──┬──┼──┬──┬──┬──┬──┬──┼──┬──┬──┬──┬──┬──┼──┬──┬──┬──┬──┬──┤  1ms tick
          │                │                 │                 │                 │
          │                │                 │                 │                 │
MQTT(4)   ├──[recv]────────┼──[recv]─────────┼──[recv]─────────┼──[recv]─────────┤  运行态
          │  500ms轮询      │  500ms轮询       │  500ms轮询       │  500ms轮询       │
          │                │                 │                 │                 │
          │                │  ┌─ DHT11(5ms)   │                 │  ┌─ DHT11       │
sensor(2) ├─────────────────┼──┤              ├──────────────────┼──┤              │  就绪态(等延时)
          │  vTaskDelay(1000)│  └──→ xQueueSend                 │  └──→ xQueueSend │
          │                │        ↓                          │        ↓         │
          │                │    g_sensor_queue                  │    g_sensor_queue │
          │                │                 │                 │                 │
          │   PRI=4 > 2 ──→  MQTT 随时抢占 sensor               │                 │
          │                │                 │                 │                 │
LED(1)    ├────────[blink]──┼────────[blink]──┼────────[blink]──┼────────[blink]──┤  250ms周期
          │                │                 │                 │                 │
          │                │                 │                 │                 │
Idle      ├[WFI]──[WFI]────┼[WFI]──[WFI]─────┼[WFI]──[WFI]─────┼[WFI]──[WFI]─────┤  CPU休眠
          │  所有任务阻塞=自动休眠            │                 │                 │
```
...
```
         ──────── 阶段 A: 接管 ──────   ─── 阶段 B: 接收 ────   ─ 阶段 C: 收尾 ─

时间     0s        1s        2s        3s        4s        5s        6s          ...

脚本     │ota_start│                    等待 6 秒...           │35×OTAD块 │ota_end│
         │size+crc │                    (设备擦除+接收)        │每50ms    │       │
         └──┬──────┘                                          │          │       │
            │  MQTT 入队                                       │          │       │
MQTT(4)  ──┤[入队]────────────────────────────────────[恢复运行]──[接收]──[接收]──
            │  ↓                                             │  入队│   入队│
            │  ota_takeover():                                │      ↓  │      ↓
            │  ① vTaskSuspend(mqtt)  ┌─ 擦除 Download ─┐      │  g_ota_queue │
OTA(31)  ──┤  ② vTaskPrioritySet(31) │ 116页×40ms=4.5s │      │      │       │
            │                        │ CPU STALL       │      │      ↓       │
            │                        └─────────────────┘      │  [写Flash]    │
            │                           ota_resume_mqtt():     │  每块~9ms     │
            │                           ① vTaskResume(mqtt)   │  PRI=31 抢占MQTT
            │                           ② OTA 保持 PRI=31      │              │
            │                                                 │              │
sensor(2)  ┤[LCD暂停]─────────────────────────────────────────┤[LCD恢复]──────
            │  ota_get_state()!=IDLE                          │              │
            │                                                 │              │
            │                                                 │ CRC+Flag+3s  │
            │                                                 └──→ 软复位 ──→ Bootloader → 新App
...
### 5.3 OTA 单帧处理微观时序 (设备端: MQTT 收包 → OTA 写 Flash)

```
mqtt_task(4)                         ota_task(31)
══════════                            ═══════════
    │                                      │ xQueueReceive() 阻塞
    │ transport_recv() → mqtt_parse()     │
    │ _cb_publish() → mqtt_on_cmd()       │
    │ ota_handle_packet():                │
    │   memcpy → queue_item               │
    │   xQueueSendToBack(g_ota_queue) ──→ │ 唤醒! (PRI=31 > 4, 抢占 MQTT)
    │                                      ├─ ota_process_data_chunk()
    │                                      │   ota_write_chunk_to_flash(~9ms)
    │                                      │   LCD 进度条更新
    │                                      └─ xQueueReceive() 阻塞 → MQTT 恢复
    │ transport_recv() → ...              │
    │ 收下一个包                            │
    │                                      │ ← 下次唤醒
```

### 5.4 Bootloader 逐页搬运时序 (31 页 × ~50ms/页, 流水线)

```
         Page N               Page N+1              Page N+2
         ┌──────┐             ┌──────┐              ┌──────┐
源码     │[读2KB]│              │[读2KB]│               │[读2KB]│
Download→RAM <1ms             Download→RAM <1ms      Download→RAM
         └──┬───┘             └──┬───┘              └──┬───┘
            ↓                    ↓                     ↓
         ┌──────┐             ┌──────┐              ┌──────┐
擦除     │[擦1页]│              │[擦1页]│               │[擦1页]│
...
总耗时: 31 × 50ms ≈ 1.5s    │  断电安全: 每页独立原子操作, 断电最多损失一页
```
